...
<template>
  <div class="space-y-4">
    ...
    <div v-if="loading" class="h-16 flex items-center justify-center text-gray-400 text-[12px]">
      <i class="fas fa-spinner fa-spin mr-2"></i> Yükleniyor...
    </div>

    <div v-else-if="benefits.length === 0" class="h-16 flex items-center justify-center bg-gray-50 rounded-xl border border-dashed border-gray-200 text-gray-400 text-[12px]">
    ...
    <div v-else class="border border-gray-100 rounded-xl overflow-hidden bg-white shadow-sm">
      <table class="w-full text-left text-[12.5px]">
        ...
        <tbody class="divide-y divide-gray-50">
          <tr v-for="b in benefits" :key="b.id" class="hover:bg-blue-50/20 transition-colors">
            <td class="px-4 py-2.5 font-semibold text-gray-800">{{ benefitTypeLabels[b.benefit_type] || b.benefit_type }}</td>
            <td class="px-4 py-2.5 font-bold text-emerald-600">{{ formatCurrency(b.amount) }}</td>
            <td class="px-4 py-2.5 text-gray-500 italic max-w-[220px] truncate" :title="b.notes">{{ b.notes || '—' }}</td>
            <td class="px-4 py-2.5 text-right">
              <button type="button" @click="deleteBenefit(b)" class="w-7 h-7 flex items-center justify-center text-gray-300 hover:text-red-500 hover:bg-red-50 rounded transition-all">
    ...
    <div class="flex items-end gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100">
      <div class="flex-1">
        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-wide mb-1">Destek Türü</label>
        <select v-model="newBenefit.benefit_type" class="w-full h-9 px-2.5 text-[12.5px] border border-gray-200 rounded-lg outline-none focus:border-blue-500 bg-white">
      ...
      <div class="w-32">
        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-wide mb-1">Aylık Tutar (₺)</label>
        <input v-model="newBenefit.amount" type="number" min="0" step="0.01" placeholder="0.00"
          class="w-full h-9 px-2.5 text-[12.5px] border border-gray-200 rounded-lg outline-none focus:border-blue-500">
      </div>
      <div class="flex-1">
        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-wide mb-1">Not (Opsiyonel)</label>
