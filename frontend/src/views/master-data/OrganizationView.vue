...
<template>
  <div class="h-full flex flex-col p-6 overflow-hidden bg-white dark:bg-slate-900">
    ...
    <div class="flex items-center justify-between mb-6 shrink-0">
      <div>
        <h1 class="text-[20px] font-bold text-gray-900 dark:text-slate-100 tracking-tight">Organizasyon Yapısı</h1>
        <p class="text-[13px] text-gray-400 dark:text-slate-500 mt-1">Şirket, Departman ve Masraf Yerlerini global olarak yönetin</p>
      </div>
    </div>
    ...
    <div class="flex-1 min-height-0">
      ...
      >
        ...
        <template #cell-company_id="{ value }">
          <span class="font-mono font-bold text-indigo-600 dark:text-indigo-400 text-[13px]">{{ value || '—' }}</span>
        </template>

        <template #cell-dept_id="{ value }">
          <span class="font-mono font-bold text-blue-600 dark:text-blue-400 text-[13px]">{{ value || '—' }}</span>
        </template>
        ...
        <template #cell-name="{ value }">
          <span class="text-[13px] font-semibold text-gray-800 dark:text-slate-100">{{ value }}</span>
        </template>

        <template #cell-created_at="{ value }">
          <span class="text-[12px] text-gray-400 dark:text-slate-500">{{ new Date(value).toLocaleDateString('tr-TR') }}</span>
        </template>
        ...
        <template #cell-personnel_count="{ row, value }">
          <button
            ...
            class="group inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 hover:bg-blue-600 hover:text-white transition-all duration-200"
          >
      ...
      <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" @click.self="showModal = false">
        <div class="bg-white dark:bg-slate-800 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
          <div class="px-7 py-5 border-b border-gray-100 dark:border-slate-700 flex items-center justify-between bg-gray-50/50 dark:bg-slate-900/50">
            <div>
              <h2 class="text-[16px] font-bold text-gray-900 dark:text-slate-100">
                {{ editingItem ? 'Kaydı Düzenle' : (activeTab === 'companies' ? 'Şirket Ekle' : activeTab === 'departments' ? 'Departman Ekle' : 'Masraf Yeri Ekle') }}
              </h2>
            </div>
            <button type="button" @click="showModal = false" class="text-gray-400 dark:text-slate-500 hover:text-gray-700 dark:hover:text-slate-200 w-8 h-8 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-700 inline-flex items-center justify-center">
          ...
          <div class="p-7 space-y-4">
            ...
            <template v-if="activeTab === 'companies'">
              <div class="grid grid-cols-4 gap-4">
                <div class="space-y-1.5">
                  <label class="text-[11px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest">ID (Oto)</label>
                  <input :value="formData.company_id" type="text" readonly placeholder="Oto"
                    class="w-full h-11 px-4 text-[13px] border border-gray-200 dark:border-slate-700 rounded-xl outline-none bg-gray-50 dark:bg-slate-900 text-gray-400 dark:text-slate-500 font-mono font-bold">
                </div>
                <div class="col-span-3 space-y-1.5">
                  <label class="text-[11px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest">Şirket Tam Adı</label>
                  <input v-model="formData.name" type="text" placeholder="Örn: Talay Lojistik A.Ş."
                    class="w-full h-11 px-4 text-[13px] border border-gray-200 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 rounded-xl outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-50/50 dark:focus:ring-blue-500/20 transition-all">
                </div>
              </div>
              <div class="grid grid-cols-2 gap-4">
                <div class="space-y-1.5">
                  <label class="text-[11px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest">Vergi Numarası</label>
                  <input v-model="formData.tax_number" type="text"
                    class="w-full h-11 px-4 text-[13px] border border-gray-200 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 rounded-xl outline-none focus:border-blue-500">
                </div>
                <div class="space-y-1.5">
                  <label class="text-[11px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest">Web Sitesi (Domain)</label>
            ...
            <template v-if="activeTab === 'departments'">
              <div class="grid grid-cols-4 gap-4">
                <div class="space-y-1.5">
                  <label class="text-[11px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest">Kod (Oto)</label>
                  <input :value="formData.dept_id" type="text" readonly placeholder="Oto"
                    class="w-full h-11 px-4 text-[13px] border border-gray-200 dark:border-slate-700 rounded-xl outline-none bg-gray-50 dark:bg-slate-900 text-gray-400 dark:text-slate-500 font-mono font-bold">
                </div>
                <div class="col-span-3 space-y-1.5">
                  <label class="text-[11px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest">Departman İsmi</label>
                  <input v-model="formData.name" type="text" placeholder="Örn: Bilgi Teknolojileri"
                    class="w-full h-11 px-4 text-[13px] border border-gray-200 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 rounded-xl outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-50/50 dark:focus:ring-blue-500/20">
                </div>
              </div>
            </template>
            ...
            <template v-if="activeTab === 'cost-centers'">
              <div class="grid grid-cols-2 gap-4">
                <div class="space-y-1.5">
                  <label class="text-[11px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest">Kod</label>
                  <input v-model="formData.code" type="text" placeholder="BT-01"
                    class="w-full h-11 px-4 text-[13px] border border-gray-200 dark:border-slate-700 dark:bg-slate-900 rounded-xl outline-none focus:border-blue-500 font-mono font-bold text-blue-600 dark:text-blue-400 uppercase">
                </div>
                <div class="space-y-1.5">
                  <label class="text-[11px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest">İsim</label>
            ...
            <div class="space-y-1.5">
              <label class="text-[11px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest">Notlar</label>
              <textarea v-model="formData.notes" rows="2"
                class="w-full p-4 text-[13px] border border-gray-200 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 rounded-xl outline-none focus:border-blue-500 resize-none"></textarea>
            </div>
          </div>
          ...
          <div class="px-7 py-4 border-t border-gray-100 dark:border-slate-700 bg-gray-50/50 dark:bg-slate-900/50 flex justify-end gap-3">
            <button type="button" @click="showModal = false" class="px-4 py-2 text-[13px] font-bold text-gray-500 dark:text-slate-400 hover:text-gray-700 dark:hover:text-slate-200">İptal</button>
    ...
    <Teleport to="body">
      <div v-if="showHistoryModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" @click.self="showHistoryModal = false">
        <div class="bg-white dark:bg-slate-800 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
          <div class="px-7 py-5 border-b border-gray-100 dark:border-slate-700 flex items-center justify-between bg-gray-50/50 dark:bg-slate-900/50">
            <div>
              <h2 class="text-[16px] font-bold text-gray-900 dark:text-slate-100">Kayıt Geçmişi & Detayları</h2>
              <p class="text-[11px] text-gray-400 dark:text-slate-500 mt-0.5">Sistem ID: {{ historyItem?.id }}</p>
            </div>
            <button type="button" @click="showHistoryModal = false" class="text-gray-400 dark:text-slate-500 hover:text-gray-700 dark:hover:text-slate-200 w-8 h-8 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-700 inline-flex items-center justify-center">
              <i class="fas fa-times"></i>
            </button>
          </div>
          ...
          <div class="p-7">
            <div class="space-y-6">
              <div class="flex items-start gap-4">
                ...
                <div>
                  <h4 class="text-[13px] font-bold text-gray-800 dark:text-slate-100">Kayıt Oluşturuldu</h4>
                  <p class="text-[12px] text-gray-500 dark:text-slate-400 mt-1">İşlem Tarihi: <span class="font-semibold text-gray-700 dark:text-slate-300">{{ historyItem?.created_at ? new Date(historyItem.created_at).toLocaleString('tr-TR') : 'Belirtilmemiş' }}</span></p>
                  <p class="text-[11px] text-gray-400 dark:text-slate-500 mt-1 italic">Sistem tarafından otomatik oluşturuldu.</p>
                </div>
              </div>

              <div class="flex items-start gap-4">
                <div class="w-10 h-10 rounded-xl bg-gray-50 dark:bg-slate-700 flex items-center justify-center shrink-0">
                  <i class="fas fa-info-circle text-gray-400 dark:text-slate-400"></i>
                </div>
                <div class="flex-1">
                  <h4 class="text-[13px] font-bold text-gray-800 dark:text-slate-100">Mevcut Veriler</h4>
                  <div class="mt-2 space-y-2">
                    <div class="flex justify-between py-1 border-b border-gray-50 dark:border-slate-700">
                      <span class="text-[12px] text-gray-400 dark:text-slate-500">İsim / Açıklama:</span>
                      <span class="text-[12px] font-semibold text-gray-700 dark:text-slate-300">{{ historyItem?.name }}</span>
                    </div>
                    <div v-if="historyItem?.dept_id" class="flex justify-between py-1 border-b border-gray-50 dark:border-slate-700">
                      <span class="text-[12px] text-gray-400 dark:text-slate-500">Departman Kodu:</span>
                      <span class="text-[12px] font-bold text-blue-600 dark:text-blue-400 font-mono">{{ historyItem?.dept_id }}</span>
                    </div>
                    <div v-if="historyItem?.company_id" class="flex justify-between py-1 border-b border-gray-50 dark:border-slate-700">
                      <span class="text-[12px] text-gray-400 dark:text-slate-500">Şirket ID:</span>
                      <span class="text-[12px] font-bold text-indigo-600 dark:text-indigo-400 font-mono">{{ historyItem?.company_id }}</span>
                    </div>
                    <div v-if="historyItem?.tax_number" class="flex justify-between py-1 border-b border-gray-50 dark:border-slate-700">
                      <span class="text-[12px] text-gray-400 dark:text-slate-500">Vergi No:</span>
                      <span class="text-[12px] font-semibold text-gray-700 dark:text-slate-300">{{ historyItem?.tax_number }}</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div class="mt-8 p-4 bg-amber-50 dark:bg-amber-500/10 rounded-xl border border-amber-100/50 dark:border-amber-500/20">
          ...
          <div class="px-7 py-4 border-t border-gray-100 dark:border-slate-700 bg-gray-50/50 dark:bg-slate-900/50 flex justify-end">
            <button type="button" @click="showHistoryModal = false" class="px-6 py-2 bg-gray-900 dark:bg-slate-700 text-white text-[13px] font-bold rounded-xl hover:bg-black dark:hover:bg-slate-600 shadow-sm">
              Kapat
            </button>
          </div>
        </div>
