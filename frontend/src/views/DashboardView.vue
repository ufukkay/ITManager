...
<template>
  <div class="px-8 py-10 max-w-5xl mx-auto">
    ...
    <!-- Karşılama -->
    <div class="mb-10">
      <div class="text-[13px] text-gray-500 dark:text-slate-400">Merhaba {{ authStore.userName.split(' ')[0] }},</div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10">
      ...
      <router-link to="/helpdesk" class="group relative overflow-hidden p-6 rounded-2xl border border-gray-200/80 bg-gradient-to-br from-white to-blue-50/20 dark:from-slate-800 dark:to-slate-900/50 dark:border-slate-700/50 hover:border-blue-400 hover:shadow-lg dark:hover:border-blue-500 transition-all duration-300 flex flex-col justify-between h-32">
        ...
        <div class="flex items-baseline gap-2 mt-2">
          ...
          <span class="text-[11px] font-medium text-gray-400 dark:text-slate-500">aktif talep</span>
        </div>
      </router-link>
      ...
      <router-link to="/monitoring" class="group relative overflow-hidden p-6 rounded-2xl border border-gray-200/80 bg-gradient-to-br from-white to-red-50/20 dark:from-slate-800 dark:to-slate-900/50 dark:border-slate-700/50 hover:border-red-400 hover:shadow-lg dark:hover:border-red-500 transition-all duration-300 flex flex-col justify-between h-32">
        <div class="flex items-center justify-between">
          <span class="text-[12px] font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Aktif Sistem Alarmları</span>
        ...
        <div class="flex items-baseline gap-2 mt-2">
          ...
          <span class="text-[11px] font-medium text-gray-400 dark:text-slate-500">kritik uyarı</span>
        </div>
      </router-link>
      ...
      <router-link to="/cost-management" class="group relative overflow-hidden p-6 rounded-2xl border border-gray-200/80 bg-gradient-to-br from-white to-emerald-50/20 dark:from-slate-800 dark:to-slate-900/50 dark:border-slate-700/50 hover:border-emerald-400 hover:shadow-lg dark:hover:border-emerald-500 transition-all duration-300 flex flex-col justify-between h-32">
        <div class="flex items-center justify-between">
          <span class="text-[12px] font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider">{{ stats.latestPeriod ? stats.latestPeriod + ' Dönemi Maliyeti' : 'Aylık Toplam Maliyet' }}</span>
        ...
        <div class="flex items-baseline gap-1 mt-2">
          ...
          <span class="text-[12px] font-bold text-gray-400 dark:text-slate-500">₺</span>
        </div>
      </router-link>
    </div>
    ...
    <!-- Modüller -->
    <div class="mb-8">
      <div class="text-[11px] uppercase tracking-wider text-gray-400 dark:text-slate-500 font-semibold mb-3">Modüller</div>
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        ...
        <router-link
          ...
          :key="m.title"
          :to="m.href"
          class="group text-left p-4 rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 flex items-center gap-3 hover:border-gray-300 dark:hover:border-slate-600 hover:shadow-sm transition-all"
        >
          <div class="w-10 h-10 rounded-lg bg-gray-50 dark:bg-slate-700 flex items-center justify-center shrink-0 text-gray-500 dark:text-slate-400 group-hover:text-gray-900 dark:group-hover:text-white transition-colors">
            <i :class="['fas text-[15px]', m.icon]"></i>
          </div>
          <div class="flex-1 min-w-0">
            <div class="text-[14px] font-semibold text-gray-900 dark:text-white">{{ m.title }}</div>
            <div class="text-[12px] text-gray-500 dark:text-slate-400 truncate">{{ m.desc }}</div>
          </div>
          <div class="text-[11px] font-medium text-gray-500 dark:text-slate-400 shrink-0">{{ m.count }}</div>
        </router-link>
        ...
        <div
          v-for="m in filteredModules.filter(m => m.soon)"
          :key="m.title"
          class="p-4 rounded-xl border border-gray-100 dark:border-slate-800 bg-gray-50 dark:bg-slate-800/40 flex items-center gap-3 cursor-not-allowed"
        >
          <div class="w-10 h-10 rounded-lg bg-gray-100 dark:bg-slate-900 flex items-center justify-center shrink-0 text-gray-300 dark:text-slate-700">
            <i :class="['fas text-[15px]', m.icon]"></i>
          </div>
          <div class="flex-1 min-w-0">
            <div class="text-[14px] font-semibold text-gray-400 dark:text-slate-600">{{ m.title }}</div>
            <div class="text-[12px] text-gray-400 dark:text-slate-600 truncate">{{ m.desc }}</div>
          </div>
          <div class="text-[11px] font-medium text-gray-400 dark:text-slate-600 shrink-0">{{ m.count }}</div>
        </div>
    ...
    <!-- Son Etkinlik -->
    <div>
      <div class="text-[11px] uppercase tracking-wider text-gray-400 dark:text-slate-500 font-semibold mb-3">Son Etkinlik</div>
      <div class="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl divide-y divide-gray-100 dark:divide-slate-700 overflow-hidden shadow-sm">
        <div v-if="activities.length === 0" class="p-8 text-center text-gray-400 dark:text-slate-500 text-[13px]">
          Henüz bir aktivite kaydı bulunmuyor.
        </div>
        <div
          v-for="(a, i) in activities"
          :key="a.id"
          class="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors"
        >
          <div
            class="w-8 h-8 rounded-full flex items-center justify-center shrink-0 bg-gray-50 dark:bg-slate-700 text-gray-500 dark:text-slate-400"
          >
            <i :class="['fas text-[12px]', getIcon(a.module)]"></i>
          </div>
          <div class="flex-1 min-w-0">
            <div class="text-[13px] text-gray-700 dark:text-slate-200 truncate">{{ formatAction(a) }}</div>
            <div class="text-[11px] text-gray-400 dark:text-slate-500">{{ a.user_name || 'Sistem' }} tarafından</div>
          </div>
          <div class="text-[12px] text-gray-400 dark:text-slate-500 shrink-0 font-medium">{{ getTimeAgo(a.created_at) }}</div>
        </div>
      </div>
    </div>
