  ...
  const ws = XLSX.utils.json_to_sheet([
    {
      ...
      'Departman': 'Operasyon',
      'Durum': 'Aktif',
      'Notlar': 'Araç takip sistemi için.'
    }
  ])
  const wb = XLSX.utils.book_new()
  XLSX.utils.book_append_sheet(wb, ws, 'Sablon')
  XLSX.writeFile(wb, 'M2M_Yukleme_Sablonu.xlsx')
}
    ...
    try {
      ...
      for (const row of rows) {
        try {
          ...
          // 5. Vehicle / Plaka
          const plateNo = String(getVal(row, 'Araç / Plaka', 'Plaka', 'Plate', 'Vehicle') || '').trim()
          let vehicleId = null
          if (plateNo) {
            let veh = masterData.vehicles.find(v => v.plate_no.toLowerCase() === plateNo.toLowerCase())
            if (!veh) {
              const newVeh = await masterData.createItem('vehicles', { plate_no: plateNo })
              vehicleId = newVeh.id
      ...
      if (successCount > 0) {
        showToast(`${successCount} M2M hattı başarıyla aktarıldı.`, 'success')
      }
      if (errorCount > 0) {
      ...
      <template #toolbar>
        ...
        <div class="ml-auto flex items-center gap-2">
          ...
          <button type="button" @click="openAddModal"
            ...
            <i class="fas fa-plus text-[11px]"></i> Yeni M2M Ekle
          </button>
        </div>
      </template>
