...
const filteredPersonnel = computed(() => {
    if (!listSearchQuery.value) return masterData.personnel
    const q = listSearchQuery.value.toLowerCase()
    return masterData.personnel.filter(p => `${p.first_name} ${p.last_name}`.toLowerCase().includes(q) || (p.company_name || '').toLowerCase().includes(q))
})
...
const vehicles = ref([])
const locations = ref([])
const personnel = computed(() => masterData.personnel)
const departments = computed(() => masterData.departments)
...
const fetchData = async () => {
    ...
    try {
        await Promise.all([
            ...
            (async () => { operators.value = (await api.get('/sim-takip/api/operators')).data })(),
            (async () => { packages.value = (await api.get('/sim-takip/api/packages')).data })(),
            (async () => { vehicles.value = (await api.get('/sim-takip/api/vehicles')).data })(),
            (async () => { locations.value = (await api.get('/sim-takip/api/locations')).data })()
        ])
    } finally {
        loading.value = false
    }
...
<template>
    <div class="h-full flex flex-col bg-[#f8f9fa] overflow-hidden">
        ...
        <div class="flex-1 overflow-hidden flex flex-col p-4">
            ...
            <div class="flex-1 bg-white rounded border border-gray-200 shadow-sm flex flex-col overflow-hidden">
                ...
                <div v-if="activeTab === 'simcards'" class="flex-1 flex flex-col bg-white">
                    ...
                    <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-white">
                        ...
                        <div class="flex bg-gray-50 p-1 rounded-lg border border-gray-100">
                            <button v-for="m in [{id:'transfer', n:'Hat Aktar', i:'fa-exchange-alt'}, {id:'single', n:'Yeni Hat', i:'fa-plus'}, {id:'bulk', n:'Excel', i:'fa-file-excel'}]" 
                                ...
                                class="flex items-center gap-2 px-4 py-1.5 rounded-md text-[13px] font-bold transition-all"
                    ...
                    <div v-if="uploadMode === 'transfer'" class="flex-1 flex flex-col overflow-hidden">
                        ...
                        <div class="p-6 border-b border-gray-50 bg-gray-50/30">
                            <div class="max-w-2xl mx-auto relative">
                                ...
                                <input v-model="searchQuery" @input="searchSims" type="text"
                                    ...
                                    class="w-full h-12 bg-white border border-gray-200 pl-11 pr-4 rounded focus:border-blue-500 focus:ring-4 focus:ring-blue-50 outline-none text-[14px] transition-all">
                                
                                <!-- Search Results Dropdown -->
                                <div v-if="searchResults.length > 0" class="absolute z-50 left-0 right-0 mt-2 bg-white rounded border border-gray-200 shadow-sm overflow-hidden divide-y divide-gray-50">
                                    <button v-for="res in searchResults" :key="res.id + res.type" 
                                        ...
                                        <div class="flex items-center gap-4">
                                            <div class="w-9 h-9 rounded-lg flex items-center justify-center font-bold text-[12px] uppercase bg-gray-100 text-gray-500">
                                                {{ res.type }}
                                            </div>
                                            <div class="flex flex-col">
                                                <span class="text-[14px] font-bold text-gray-700 leading-none mb-1">{{ res.phone_no || 'No Yok' }}</span>
                                                <span class="text-[13px] text-gray-400 font-mono">{{ res.iccid || 'ICCID Yok' }}</span>
                                            </div>
                                        </div>
                                        <div class="text-right">
                                            ...
                                            <span class="text-[12px] text-gray-400 uppercase ">{{ res.operator }}</span>
                                        </div>
                                    </button>
                                </div>
                            </div>
                        ...
                        <div class="flex-1 overflow-y-auto p-6">
                            <div v-if="selectedSim" class="max-w-4xl mx-auto grid grid-cols-2 gap-6">
                                <!-- LEFT: Current State -->
                                <div class="space-y-4">
                                    <h3 class="text-[12px] font-bold text-gray-400 uppercase tracking-wide ml-1">MEVCUT DURUM</h3>
                                    <div class="bg-gray-50/50 p-6 rounded border border-gray-100 space-y-6">
                                        <div class="flex items-center gap-4">
                                            <div class="w-12 h-12 rounded bg-white border border-gray-100 flex items-center justify-center text-gray-400">
                                                <i class="fas fa-sim-card text-xl"></i>
                                            </div>
                                            <div>
                                                <p class="text-[12px] text-gray-400 font-bold uppercase tracking-normal">{{ selectedSim.type }} HATTI</p>
                                                <p class="text-[18px] font-bold text-gray-800">{{ selectedSim.phone_no }}</p>
                                            </div>
                                        </div>
                                        <div class="p-4 bg-white border border-gray-100 rounded">
                                            <p class="text-[12px] text-gray-400 font-bold uppercase mb-2">Atandığı Yer</p>
                                            <div class="flex items-center gap-3">
                                ...
                                <div class="space-y-4">
                                    <h3 class="text-[12px] font-bold text-gray-400 uppercase tracking-wide ml-1">YENİ HEDEF</h3>
                                    <div class="bg-white p-6 rounded border border-blue-100 shadow-sm ring-4  space-y-6">
                                        <div class="grid grid-cols-2 gap-2">
                                            <button v-for="t in ['vehicle', 'personnel', 'location', 'stock']" :key="t"
                                                @click="transferTargetType = t; transferTargetId = ''"
                                                class="py-2.5 rounded-lg text-[13px] font-bold border transition-all"
                                        ...
                                        <button @click="performTransfer" :disabled="loading" 
                                            class="w-full h-11 bg-blue-600 text-white rounded-lg font-bold uppercase tracking-wide hover:bg-blue-700 transition-colors shadow-sm shadow-blue-200">
                    ...
                    <div v-else-if="uploadMode === 'single'" class="flex-1 overflow-y-auto p-8 bg-white">
                        <div class="max-w-xl mx-auto space-y-8">
                            ...
                            <div class="grid grid-cols-2 gap-6">
                                <div class="space-y-2">
                                    <label class="text-[12px] font-bold text-gray-400 uppercase ml-1">TELEFON</label>
                                ...
                                <div class="space-y-2">
                                    <label class="text-[12px] font-bold text-gray-400 uppercase ml-1">ICCID</label>
                            ...
                            <div class="grid grid-cols-2 gap-6">
                                <div class="space-y-2">
                                    <label class="text-[12px] font-bold text-gray-400 uppercase ml-1">TİP</label>
                                ...
                                <div class="space-y-2">
                                    <label class="text-[12px] font-bold text-gray-400 uppercase ml-1">OPERATÖR</label>
                            ...
                            <button @click="saveSim" :disabled="loading" 
                                class="w-full h-12 bg-blue-600 text-white rounded-lg font-bold shadow-sm hover:bg-blue-700 transition-all uppercase tracking-wide">
                                Sisteme Kaydet
                            </button>
                        </div>
                    </div>
                    ...
                    <div v-else-if="uploadMode === 'bulk'" class="flex-1 flex flex-col items-center justify-center p-8">
                        <div class="max-w-md w-full bg-white p-10 rounded border-2 border-dashed border-gray-100 flex flex-col items-center text-center space-y-6">
                            <div class="w-16 h-16 bg-green-50 rounded flex items-center justify-center text-green-600 mb-2">
                ...
                <div v-else class="flex-1 flex flex-col overflow-hidden bg-white">
                    ...
                    <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-white">
                        <div class="flex items-center gap-3">
                            <div class="w-9 h-9 rounded flex items-center justify-center text-white bg-gradient-to-br from-blue-500 to-blue-600 shadow-sm shadow-blue-200">
                            ...
                            <div>
                                ...
                                <p class="text-[13px] text-gray-400">Kayıt ekle, düzenle veya kaldır</p>
                            </div>
                        </div>
                        <div class="flex items-center gap-3">
                            <div class="text-[13px] font-bold text-blue-600 bg-blue-50 px-4 py-1.5 rounded-full border border-blue-100">
                    ...
                    <div class="flex-1 overflow-y-auto p-6 bg-gray-50/30">
                        <div class="max-w-4xl mx-auto space-y-6">
                            ...
                            <!-- Edit Mode Banner -->
                            <div v-if="editingItem" class="bg-amber-50 border border-amber-200 rounded p-4 flex items-center justify-between animate-fade-in">
                                <div class="flex items-center gap-3">
                                ...
                                <button @click="cancelEdit" class="text-[13px] text-amber-600 font-bold uppercase px-3 py-1.5 hover:bg-amber-100 rounded-lg transition-colors">
                            ...
                            <div class="bg-white p-6 rounded border border-gray-200 shadow-sm space-y-4">
                                <h3 class="text-[13px] font-bold text-gray-400 uppercase tracking-wide flex items-center gap-2">
                                ...
                                <div v-if="activeTab === 'operators'" class="flex gap-3">
                                    <input v-model="newOperator" type="text" placeholder="Operatör adı..."
                                        class="flex-1 h-11 bg-gray-50 border border-gray-200 px-4 rounded focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-50 outline-none text-[13px] transition-all">
                                    <button @click="addOperator"
                                        class="bg-blue-600 text-white px-8 h-11 rounded font-bold text-[12px] hover:bg-blue-700 transition-all uppercase shadow-sm shadow-blue-200 hover:shadow-blue-300">
                                ...
                                <div v-else-if="activeTab === 'companies'" class="space-y-3">
                                    <div class="flex gap-3">
                                        ...
                                        <button @click="addCompany"
                                            class="bg-blue-600 text-white px-8 h-11 rounded font-bold text-[12px] hover:bg-blue-700 transition-all uppercase shadow-sm shadow-blue-200">
                                            {{ editingItem ? 'Güncelle' : 'Ekle' }}
                                        </button>
                                    </div>
                                    <input v-model="newCompany.notes" type="text" placeholder="Kısa not veya açıklama..."
                                        class="w-full h-11 bg-gray-50 border border-gray-200 px-4 rounded focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-50 outline-none text-[13px] transition-all">
                                </div>
                                ...
                                <div v-else-if="activeTab === 'packages'" class="space-y-3">
                                    <div class="flex gap-3">
                                        <input v-model="newPackage.name" type="text" placeholder="Paket adı (örn: 10GB)"
                                            class="flex-1 h-11 bg-gray-50 border border-gray-200 px-4 rounded focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-50 outline-none text-[13px] transition-all">
                                        <select v-model="newPackage.type" class="w-32 h-11 bg-gray-50 border border-gray-200 px-3 rounded outline-none text-[13px] cursor-pointer">
                                    ...
                                    <div class="flex gap-3">
                                        <select v-model="newPackage.operator_id" class="flex-1 h-11 bg-gray-50 border border-gray-200 px-4 rounded outline-none text-[13px] cursor-pointer">
                                        ...
                                        <input v-model="newPackage.price" type="number" placeholder="Birim Fiyat"
                                            class="w-32 h-11 bg-gray-50 border border-gray-200 px-4 rounded outline-none text-[13px]">
                                ...
                                <div v-else-if="activeTab === 'vehicles'" class="flex gap-3">
                                    <input v-model="newVehicle.plate_no" type="text" placeholder="Plaka (Örn: 34ABC123)"
                                        class="flex-1 h-11 bg-gray-50 border border-gray-200 px-4 rounded focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-50 outline-none text-[13px] transition-all">
                                    <select v-model="newVehicle.vehicle_type" class="w-40 h-11 bg-gray-50 border border-gray-200 px-3 rounded outline-none text-[13px] cursor-pointer">
                                ...
                                <div v-else-if="activeTab === 'personnel'" class="space-y-4">
                                    <div class="grid grid-cols-2 gap-3">
                                        <input v-model="newPerson.first_name" type="text" placeholder="Ad"
                                            class="h-11 bg-gray-50 border border-gray-200 px-4 rounded focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-50 outline-none text-[13px] transition-all">
                                        <input v-model="newPerson.last_name" type="text" placeholder="Soyad"
                                            class="h-11 bg-gray-50 border border-gray-200 px-4 rounded focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-50 outline-none text-[13px] transition-all">
                                    </div>
                                    <div class="grid grid-cols-2 gap-3">
                                        <div class="flex gap-2">
                                            <select v-model="newPerson.company_id" class="flex-1 h-11 bg-gray-50 border border-gray-200 px-4 rounded outline-none text-[13px] cursor-pointer">
                                                <option value="">Şirket Seçin...</option>
                                                <option v-for="c in companies" :key="c.id" :value="c.id">{{ c.name }}</option>
                                            </select>
                                            <button @click="activeTab = 'companies'" class="w-11 h-11 bg-gray-100 text-gray-400 rounded hover:bg-blue-50 hover:text-blue-600 transition-all">
                                                <i class="fas fa-plus"></i>
                                            </button>
                                        </div>
                                        <div class="flex gap-2">
                                            <select v-model="newPerson.department_id" class="flex-1 h-11 bg-gray-50 border border-gray-200 px-4 rounded outline-none text-[13px] cursor-pointer">
                                                <option value="">Departman Seçin...</option>
                                                <option v-for="d in departments" :key="d.id" :value="d.id">{{ d.name }}</option>
                                            </select>
                                            <button @click="activeTab = 'departments'" class="w-11 h-11 bg-gray-100 text-gray-400 rounded hover:bg-blue-50 hover:text-blue-600 transition-all">
                                                <i class="fas fa-plus"></i>
                                            </button>
                                        </div>
                                    </div>
                                    <button @click="addPerson" class="w-full h-11 bg-blue-600 text-white rounded font-bold uppercase tracking-wide hover:bg-blue-700 transition-colors shadow-sm shadow-blue-200">
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="relative">
                                    ...
                                    <input v-model="listSearchQuery" type="text" placeholder="Kayıtlarda ara..."
                                        class="w-full h-10 bg-white border border-gray-200 pl-10 pr-4 rounded focus:border-blue-500 focus:ring-4 focus:ring-blue-50 outline-none text-[13px] transition-all">
                                    <div v-if="listSearchQuery" class="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2">
                                        <span class="text-[12px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full">{{ currentListData.length }}/{{ totalListCount }}</span>
                                        <button @click="listSearchQuery = ''" class="text-gray-400 hover:text-gray-600"><i class="fas fa-times text-[12px]"></i></button>
                                    </div>
                                </div>
                                ...
                                <div class="px-2 pb-1 flex items-center justify-between">
                                    <div class="flex items-center gap-2 cursor-pointer group" @click="toggleSelectAll">
                                        <div class="w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all shadow-sm"
                                            ...
                                            <i v-if="selectedIds.length === currentListData.length && currentListData.length > 0" class="fas fa-check text-[12px]"></i>
                                            <div v-else-if="selectedIds.length > 0" class="w-2 h-0.5 bg-blue-400 rounded-full"></div>
                                        </div>
                                        <span class="text-[13px] font-bold text-gray-500 group-hover:text-blue-600 transition-colors uppercase tracking-normal">Tümünü Seç ({{ currentListData.length }})</span>
                                    </div>
                                    <span v-if="selectedIds.length > 0" class="text-[12px] font-bold text-blue-500 bg-blue-50 px-2 py-0.5 rounded-md border border-blue-100">
                                        {{ selectedIds.length }} SEÇİLİ
                                    </span>
                                </div>

                                <!-- List -->
                                <div v-if="currentListData.length > 0" class="bg-white rounded border border-gray-200 overflow-hidden shadow-sm">
                                        ...
                                        <div v-for="(op, idx) in filteredOperators" :key="op.id" @click="toggleSelect(op.id)" class="group px-5 py-3.5 flex justify-between items-center hover:bg-blue-50/30 transition-all border-b border-gray-50 last:border-0 cursor-pointer">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div class="w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all shrink-0"
                                                    ...
                                                    <i v-if="selectedIds.includes(op.id)" class="fas fa-check text-[12px]"></i>
                                                </div>
                                                <span class="text-[12px] font-bold text-gray-300 w-5 text-right">{{ idx + 1 }}</span>
                                                <div class="w-8 h-8 rounded-lg bg-gray-50 flex items-center justify-center text-gray-400"><i class="fas fa-broadcast-tower text-[13px]"></i></div>
                                        ...
                                        <div v-for="(comp, idx) in filteredCompanies" :key="comp.id" @click="toggleSelect(comp.id)" class="group px-5 py-3.5 flex justify-between items-center hover:bg-blue-50/30 transition-all border-b border-gray-50 last:border-0 cursor-pointer">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div class="w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all shrink-0"
                                                    ...
                                                    <i v-if="selectedIds.includes(comp.id)" class="fas fa-check text-[12px]"></i>
                                                </div>
                                                <span class="text-[12px] font-bold text-gray-300 w-5 text-right">{{ idx + 1 }}</span>
                                                <div class="w-8 h-8 rounded-lg bg-gradient-to-br from-blue-50 to-blue-100 flex items-center justify-center text-blue-400"><i class="fas fa-building text-[13px]"></i></div>
                                                <div class="flex flex-col">
                                                    <span class="text-[13px] font-semibold text-gray-700 leading-tight">{{ comp.name }}</span>
                                                    <span class="text-[13px] text-gray-400">{{ comp.notes || '—' }}</span>
                                                </div>
                                            </div>
                                            <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        ...
                                        <div v-for="(pkg, idx) in filteredPackages" :key="pkg.id" @click="toggleSelect(pkg.id)" class="group px-5 py-3.5 flex justify-between items-center hover:bg-blue-50/30 transition-all border-b border-gray-50 last:border-0 cursor-pointer">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div class="w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all shrink-0"
                                                    ...
                                                    <i v-if="selectedIds.includes(pkg.id)" class="fas fa-check text-[12px]"></i>
                                                </div>
                                                <span class="text-[12px] font-bold text-gray-300 w-5 text-right">{{ idx + 1 }}</span>
                                                <div class="w-8 h-8 rounded-lg flex items-center justify-center" :class="pkg.type === 'm2m' ? 'bg-gradient-to-br from-blue-50 to-blue-100 text-blue-400' : pkg.type === 'data' ? 'bg-gray-50 text-gray-400' : 'bg-gray-50 text-gray-400'"><i class="fas fa-box-open text-[13px]"></i></div>
                                                <div class="flex flex-col">
                                                    <span class="text-[13px] font-semibold text-gray-700 leading-tight">{{ pkg.name }}</span>
                                                    <span class="text-[13px] text-gray-400">{{ pkg.operator_name }} · <span class="text-gray-700 font-bold">{{ pkg.price }}₺</span></span>
                                                </div>
                                            </div>
                                        ...
                                        <div v-for="(v, idx) in filteredVehicles" :key="v.id" @click="toggleSelect(v.id)" class="group px-5 py-3.5 flex justify-between items-center hover:bg-blue-50/30 transition-all border-b border-gray-50 last:border-0 cursor-pointer">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div class="w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all shrink-0"
                                                    ...
                                                    <i v-if="selectedIds.includes(v.id)" class="fas fa-check text-[12px]"></i>
                                                </div>
                                                <span class="text-[12px] font-bold text-gray-300 w-5 text-right">{{ idx + 1 }}</span>
                                                <div class="w-8 h-8 rounded-lg bg-gradient-to-br from-amber-50 to-amber-100 flex items-center justify-center text-amber-500"><i class="fas fa-truck-moving text-[13px]"></i></div>
                                                <div class="flex flex-col">
                                                    <span class="text-[13px] font-bold text-gray-700 leading-tight uppercase tracking-wide">{{ v.plate_no }}</span>
                                                    <span class="text-[13px] text-gray-400">{{ v.vehicle_type }}</span>
                                                </div>
                                            </div>
                                            <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        ...
                                        <div v-for="(l, idx) in filteredLocations" :key="l.id" @click="toggleSelect(l.id)" class="group px-5 py-3.5 flex justify-between items-center hover:bg-blue-50/30 transition-all border-b border-gray-50 last:border-0 cursor-pointer">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div class="w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all shrink-0"
                                                    ...
                                                    <i v-if="selectedIds.includes(l.id)" class="fas fa-check text-[12px]"></i>
                                                </div>
                                                <span class="text-[12px] font-bold text-gray-300 w-5 text-right">{{ idx + 1 }}</span>
                                                <div class="w-8 h-8 rounded-lg bg-gradient-to-br from-emerald-50 to-emerald-100 flex items-center justify-center text-emerald-400"><i class="fas fa-map-marker-alt text-[13px]"></i></div>
                                                <div class="flex flex-col">
                                                    <span class="text-[13px] font-semibold text-gray-700 leading-tight">{{ l.name }}</span>
                                                    <span class="text-[13px] text-gray-400">{{ l.address || 'Adres bilgisi yok' }}</span>
                                                </div>
                                            </div>
                                            <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        ...
                                        <div v-for="(p, idx) in filteredPersonnel" :key="p.id" @click="toggleSelect(p.id)" class="group px-5 py-3.5 flex justify-between items-center hover:bg-blue-50/30 transition-all border-b border-gray-50 last:border-0 cursor-pointer">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div class="w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all shrink-0"
                                                    ...
                                                    <i v-if="selectedIds.includes(p.id)" class="fas fa-check text-[12px]"></i>
                                                </div>
                                                <span class="text-[12px] font-bold text-gray-300 w-5 text-right">{{ idx + 1 }}</span>
                                                <div class="w-8 h-8 rounded-lg bg-gray-50 flex items-center justify-center text-gray-400"><i class="fas fa-user-tie text-[13px]"></i></div>
                                                <div class="flex flex-col">
                                                    <span class="text-[13px] font-semibold text-gray-700 leading-tight">{{ p.first_name }} {{ p.last_name }}</span>
                                                    <span class="text-[13px] text-gray-400">{{ p.company_name || '—' }} · {{ p.department_name || '—' }}</span>
                                                </div>
                                            </div>
                                            <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        ...
                                        <div v-for="(d, idx) in filteredDepartments" :key="d.id" @click="toggleSelect(d.id)" class="group px-5 py-3.5 flex justify-between items-center hover:bg-blue-50/30 transition-all border-b border-gray-50 last:border-0 cursor-pointer">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div class="w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all shrink-0"
                                                    ...
                                                    <i v-if="selectedIds.includes(d.id)" class="fas fa-check text-[12px]"></i>
                                                </div>
                                                <span class="text-[12px] font-bold text-gray-300 w-5 text-right">{{ idx + 1 }}</span>
                                                <div class="w-8 h-8 rounded-lg bg-gray-50 flex items-center justify-center text-gray-400"><i class="fas fa-sitemap text-[13px]"></i></div>
                                ...
                                <!-- Empty State -->
                                <div v-else class="bg-white rounded border border-gray-200 p-12 flex flex-col items-center justify-center text-center">
        ...
        <div v-if="loading" class="fixed inset-0 z-[10000] bg-white/60 backdrop-blur-[2px] flex items-center justify-center">
            <div class="flex flex-col items-center gap-3">
                ...
                <span class="text-[13px] font-bold text-blue-600 tracking-wide animate-pulse">İŞLENİYOR...</span>
            </div>
        </div>

        <!-- Floating Bulk Action Bar -->
        <transition name="slide-up">
            <div v-if="selectedIds.length > 0" class="fixed bottom-8 left-1/2 -translate-x-1/2 z-[100] bg-gray-900 text-white px-6 py-3 rounded shadow-md flex items-center gap-6 animate-slide-up border border-white/10 ring-8 ring-black/5">
                <div class="flex items-center gap-3 border-r border-gray-700 pr-6">
                    <div class="w-8 h-8 rounded-lg bg-blue-500 flex items-center justify-center font-bold text-[14px] shadow-sm ">
                        {{ selectedIds.length }}
                    </div>
                    <span class="text-[13px] font-bold text-gray-300 uppercase tracking-normal">Kayıt Seçildi</span>
                </div>
                <div class="flex items-center gap-2">
        ...
        <transition name="modal">
            <div v-if="isBulkEditModalShow" class="fixed inset-0 z-[9998] flex items-center justify-center p-4">
                ...
                <div class="relative bg-white rounded-md shadow-md w-full max-w-md p-8 space-y-6 animate-scale-in">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            <div class="w-12 h-12 rounded bg-blue-50 flex items-center justify-center text-blue-600 shadow-sm border border-blue-100/50">
                                <i class="fas fa-edit text-xl"></i>
                            </div>
                            <div>
                                <h3 class="text-[17px] font-bold text-gray-800 tracking-normal">Toplu Düzenleme</h3>
                    ...
                    <div class="space-y-4">
                        ...
                        <template v-if="activeTab === 'personnel'">
                            <div class="space-y-2">
                                <label class="text-[12px] font-bold text-gray-400 uppercase ml-1">Yeni Şirket</label>
                                <select v-model="bulkEditForm.company_id" class="w-full h-12 bg-gray-50 border border-gray-200 px-4 rounded outline-none text-[13px] focus:bg-white focus:border-blue-500">
                                    <option value="">Değişiklik Yok</option>
                                    <option v-for="c in companies" :key="c.id" :value="c.id">{{ c.name }}</option>
                                </select>
                            </div>
                            <div class="space-y-2">
                                <label class="text-[12px] font-bold text-gray-400 uppercase ml-1">Yeni Departman</label>
                                <select v-model="bulkEditForm.department_id" class="w-full h-12 bg-gray-50 border border-gray-200 px-4 rounded outline-none text-[13px] focus:bg-white focus:border-blue-500">
                                    <option value="">Değişiklik Yok</option>
                                    <option v-for="d in departments" :key="d.id" :value="d.id">{{ d.name }}</option>
                                </select>
                            </div>
                        ...
                        <template v-else-if="activeTab === 'packages'">
                            <div class="space-y-2">
                                <label class="text-[12px] font-bold text-gray-400 uppercase ml-1">Yeni Operatör</label>
                                <select v-model="bulkEditForm.operator_id" class="w-full h-12 bg-gray-50 border border-gray-200 px-4 rounded outline-none text-[13px] focus:bg-white focus:border-blue-500">
                                    <option value="">Değişiklik Yok</option>
                                    <option v-for="op in operators" :key="op.id" :value="op.id">{{ op.name }}</option>
                                </select>
                            </div>
                            <div class="space-y-2">
                                <label class="text-[12px] font-bold text-gray-400 uppercase ml-1">Birim Fiyat Güncelle</label>
                                <input v-model="bulkEditForm.price" type="number" placeholder="Yeni fiyat..." class="w-full h-12 bg-gray-50 border border-gray-200 px-4 rounded outline-none text-[13px] focus:bg-white focus:border-blue-500">
                            </div>
                        </template>

                        <div class="p-4 bg-amber-50 rounded border border-amber-100/50">
                    ...
                    <div class="flex gap-3 pt-2">
                        <button @click="isBulkEditModalShow = false" class="flex-1 h-12 rounded border-2 border-gray-100 text-[13px] font-bold text-gray-400 hover:bg-gray-50 transition-all uppercase tracking-wide">
                            İptal
                        </button>
                        <button @click="confirmBulkEdit" class="flex-1 h-12 rounded bg-blue-600 text-white text-[13px] font-bold shadow-sm  hover:bg-blue-700 transition-all uppercase tracking-wide">
                            Güncelle
                        </button>
                    </div>
                </div>
