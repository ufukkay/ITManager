...
const fetchLocations = async () => {
    try {
        const res = await api.get('/sim-takip/api/locations')
...
const fetchPackages = async () => {
    try {
        ...
        packages.value = res.data.filter(p => p.type === 'data')
    } catch (err) {
...
const getFilteredUniqueValues = (column) => {
    ...
    if (companyFilter.value) list = list.filter(item => item.company_name === companyFilter.value)
    if (searchQuery.value) {
        const q = searchQuery.value.toLowerCase()
        list = list.filter(item => 
            (item.iccid && item.iccid.toLowerCase().includes(q)) ||
            (item.phone_no && item.phone_no.toLowerCase().includes(q)) ||
            (item.device_info && item.device_info.toLowerCase().includes(q))
        )
    }

    // Apply other column filters (Synchronized)
...
watch([searchQuery, operatorFilter, companyFilter, statusFilter, pageSize, columnFilters], () => {
    currentPage.value = 1
}, { deep: true }) // Deep watch for columnFilters
    ...
    <div class="flex items-center gap-3 mb-4 bg-white p-2 rounded-lg border border-[#e0e0e0] flex-wrap min-h-[52px]">
      ...
      <template v-else>
        <div class="relative flex-1 min-w-[300px]">
            ...
            <input v-model="searchQuery" type="text" placeholder="Numara veya ICCID ara..." class="w-full bg-white border border-[#e0e0e0] text-[13px] pl-9 pr-4 py-1.5 rounded-md focus:border-[#1a73e8] outline-none">
        </div>
        ...
        <select v-model="companyFilter" class="bg-white border border-[#e0e0e0] text-[13px] px-3 py-1.5 rounded-md outline-none focus:border-[#1a73e8] min-w-[140px] cursor-pointer">
            ...
            <table class="w-full text-left border-collapse table-fixed">
                <thead>
                    <tr class="bg-[#f8f9fa] border-b border-[#e0e0e0] text-[#5f6368] text-[11px] font-bold uppercase tracking-wider">
                        ...
                        <th v-for="col in [
                            ...
                            { key: 'location_name', label: 'Lokasyon', width: '180px' },
                            { key: 'device_info', label: 'Cihaz Bilgisi', width: '180px' },
                ...
                <tbody class="text-[13px] text-[#3c4043]">
                    <tr v-for="(item, index) in displayList" :key="item.id" 
                        ...
                        <td class="py-2 px-4 font-medium truncate text-[#3c4043] border-r border-[#e0e0e0]" :title="item.location_name">{{ item.location_name || '-' }}</td>
                        <td class="py-2 px-4 font-medium truncate text-[#3c4043] border-r border-[#e0e0e0]" :title="item.device_info">{{ item.device_info || '-' }}</td>
                        <td class="py-2 px-4 border-r border-[#e0e0e0]">
    ...
    <dialog class="modal" :class="{ 'modal-open': isModalOpen }">
        <div class="modal-box bg-white p-6 rounded-lg shadow-2xl border border-gray-100 max-w-xl">
            <div class="flex justify-between items-center mb-6 pb-4 border-b border-gray-100">
                <h3 class="font-bold text-[15px] text-gray-900">{{ selectedItem ? 'Data Hatti Düzenle' : 'Yeni Data Hattı Ekle' }}</h3>
            ...
            <form @submit.prevent="saveItem" class="space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div class="flex flex-col gap-1.5">
                        ...
                        <input v-model="form.iccid" type="text" class="w-full border border-gray-200 text-[13px] px-3 py-2 rounded-md outline-none focus:border-[#1a73e8] focus:ring-1 focus:ring-[#1a73e8]" required>
                    </div>
                    <div class="flex flex-col gap-1.5">
                        <label class="text-[12px] font-bold text-gray-500 ml-0.5">Telefon No</label>
                        <input v-model="form.phone_no" type="text" class="w-full border border-gray-200 text-[13px] px-3 py-2 rounded-md outline-none focus:border-[#1a73e8] focus:ring-1 focus:ring-[#1a73e8]">
                    </div>
                    <div class="flex flex-col gap-1.5">
                    ...
                    <div class="flex flex-col gap-1.5">
                        <label class="text-[12px] font-bold text-gray-500 ml-0.5">Lokasyon</label>
                        <select v-model="form.location_id" class="w-full border border-gray-200 text-[13px] px-3 py-2 rounded-md outline-none focus:border-[#1a73e8] focus:ring-1 focus:ring-[#1a73e8]" required>
                            <option value="">Seçiniz</option>
                            <option v-for="loc in locations" :key="loc.id" :value="loc.id">{{ loc.name }}</option>
                        </select>
                    </div>
                    <div class="flex flex-col gap-1.5">
                        <label class="text-[12px] font-bold text-gray-500 ml-0.5">Cihaz / Terminal</label>
                        <input v-model="form.device_info" type="text" class="w-full border border-gray-200 text-[13px] px-3 py-2 rounded-md outline-none focus:border-[#1a73e8] focus:ring-1 focus:ring-[#1a73e8]">
                    </div>
                    <div class="flex flex-col gap-1.5">
                        <label class="text-[12px] font-bold text-gray-500 ml-0.5">Şirket</label>
                        <select v-model="form.company_id" class="w-full border border-gray-200 text-[13px] px-3 py-2 rounded-md outline-none focus:border-[#1a73e8] focus:ring-1 focus:ring-[#1a73e8]" required>
                            <option value="">Seçiniz</option>
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <select v-model="form.package_id" class="w-full border border-gray-200 text-[13px] px-3 py-2 rounded-md outline-none focus:border-[#1a73e8] focus:ring-1 focus:ring-[#1a73e8]" required>
                            <option value="">Seçiniz</option>
