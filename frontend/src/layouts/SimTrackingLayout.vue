  ...
  <div class="flex h-full overflow-hidden bg-white">
    ...
    <aside class="w-48 bg-gray-50 border-r border-gray-200 flex flex-col shrink-0">
      <nav class="flex-1 overflow-y-auto px-2 py-4 flex flex-col gap-4">
        ...
        <!-- Hatlar -->
        <div>
          <p class="px-3 mb-2 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Hatlar</p>
          <div class="flex flex-col gap-0.5">
            <router-link
              v-for="item in [
                { to: '/sim-takip/m2m',   icon: 'fa-sim-card',  label: 'M2M' },
                { to: '/sim-takip/data',  icon: 'fa-wifi',      label: 'Data' },
                { to: '/sim-takip/voice', icon: 'fa-phone-alt', label: 'Ses' },
              ]"
              :key="item.to"
              :to="item.to"
